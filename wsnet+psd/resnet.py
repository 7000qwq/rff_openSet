...
class Bottleneck(nn.Module):  # 定义的50，101，152层resnet中每一个残差结构(有三层卷积+一个残差)
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        ...
        self.conv2 = nn.Conv1d(in_channels=out_channel, out_channels=out_channel,
                               ...
                               padding=1)  # 这里同理，如果是作为残差层的第一个残差结构，stride=2，从而将input_size缩小为原来的一般
        self.bn2 = nn.BatchNorm1d(out_channel)
        # -----------------------------------------
        self.conv3 = nn.Conv1d(in_channels=out_channel, out_channels=out_channel * self.expansion,  # 因为第三层卷积层的输出是输入的4倍
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=5, include_top=True):
        ...
        self.conv1 = nn.Conv1d(1, self.in_channel, kernel_size=7, stride=2,  # 注意哦，self.in_channel是作为第一个卷积层的输出个数
                               padding=3, bias=False)
        self.bn1 = nn.BatchNorm1d(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool1d(kernel_size=2, stride=2, padding=1)  # 最大池化操作
    ...
    def _make_layer(self, block, channel, block_num, stride=1):  # stride=1指的是stride默认是1的意思
        '''
        参数解释
        block: 残差结构的类型(basic bottle)
        channel: 残差结构中卷积层所用到卷积核的个数(定义的是第一层卷积核的个数)
        block_num： 当前残差层的残差结构的个数
        '''
        ...
        for _ in range(1, block_num):  # 遍历实现的残差结构
